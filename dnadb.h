...
class DnaDb{
    ...
    ******************************************/
    int hash(string sequence) const; // hash function for a DNA sequence. turns string to int
    void rehashData(); // function to rehash for insert() and remove()
  int locateDNA(DNA** table, string seq, prob_t probing, int capacity) const; //function to get
                                                                        // index of DNA
};
#endif
